...
gcloud compute instances create $PC_SRV \
    ...
    --tags=http-server,"$PC_SRV",lb-health-check \
    --create-disk=boot=yes,device-name="$PC_SRV",image=projects/debian-cloud/global/images/debian-11-bullseye-v20241009,mode=rw,size=10,type=pd-standard \
    --boot-disk-device-name=petclinic-disk \
...
gcloud compute instances create $SQL_SRV \
    ...
    --service-account="$GCP_SERVICE_ACCOUNT" \
    --scopes=https://www.googleapis.com/auth/devstorage.read_only,https://www.googleapis.com/auth/logging.write,https://www.googleapis.com/auth/monitoring.write,https://www.googleapis.com/auth/service.management.readonly,https://www.googleapis.com/auth/servicecontrol,https://www.googleapis.com/auth/trace.append \
    --tags=db-server,"$SQL_SRV",lb-health-check \
    --create-disk=boot=yes,device-name="$SQL_SRV",image=projects/debian-cloud/global/images/debian-11-bullseye-v20241009,mode=rw,size=10,type=pd-standard \
    --boot-disk-device-name=mysqlserver-disk \
    --no-shielded-secure-boot \
    --shielded-vtpm \
    --shielded-integrity-monitoring \
